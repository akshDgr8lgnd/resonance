...
export class SettingsService {
  ...
  get(): AppSettings {
    ...
    const youtubeKey = map.get("youtubeKey") ?? process.env.YOUTUBE_API_KEY ?? "";
    ...
    if (youtubeKey) {
      this.db.run("INSERT OR REPLACE INTO settings (key, value) VALUES ('youtubeKey', ?)", [youtubeKey]);
    }

    return { apiToken, serverPort, desktopDeviceId, youtubeKey };
  }

  updateYoutubeKey(key: string) {
    this.db.run("INSERT OR REPLACE INTO settings (key, value) VALUES ('youtubeKey', ?)", [key]);
  }
}
